    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-4">
      ...
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-slate-50 via-white to-slate-100">
        <motion.div
          ...
          className="absolute top-20 left-10 w-72 h-72 rounded-full bg-blue-900/5 blur-3xl"
        />
        <motion.div
          animate={{ x: [0, -25, 0], y: [0, 30, 0] }}
          transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-40 right-20 w-96 h-96 rounded-full bg-teal-500/5 blur-3xl"
        />
        <motion.div
          animate={{ x: [0, 20, 0], y: [0, 25, 0] }}
          transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
          className="absolute bottom-20 left-1/3 w-80 h-80 rounded-full bg-amber-500/5 blur-3xl"
        />
      </div>
      ...
      <div className="max-w-5xl mx-auto text-center">
          ...
          {["Desarrollador Web", "Desarrollador de Software", "Sistemas POS", "Ciberseguridad"].map((tag, i) => (
            <span
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium border backdrop-blur-sm ${
                ...
                i === 1 ? 'bg-teal-50 border-teal-200 text-teal-900' :
                i === 2 ? 'bg-amber-50 border-amber-200 text-amber-900' :
        ...
        >
          <a href={CV_URL} target="_blank" rel="noopener noreferrer" download>
            <Button
              size="lg"
              className="bg-gradient-to-r from-blue-900 to-teal-600 hover:from-blue-800 hover:to-teal-500 text-white rounded-full px-8 py-6 text-base font-semibold shadow-lg shadow-blue-900/30 transition-all duration-300 hover:shadow-xl hover:shadow-teal-600/30"
            >
          ...
          <a href="#proyectos">
            <Button
              size="lg"
              variant="outline"
              className="rounded-full px-8 py-6 text-base font-semibold border-2 border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400 transition-all duration-300"
            >
              Ver Proyectos
            </Button>
          </a>
